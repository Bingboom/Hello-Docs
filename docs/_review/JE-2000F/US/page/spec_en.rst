.. raw:: latex

   \HBApplyLang{en}

.. only:: latex

   .. raw:: latex

      \section{SPECIFICATIONS}

.. raw:: latex

   \specsectiontitle{GENERAL INFO}
   \begin{spectable}
   \noindent\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}m{\csname HBcomp_spec_table_left_ratio\endcsname\linewidth}|>{\raggedright\arraybackslash}X@{}}
   \HBTypeSpecLabel{Product Name} & \HBTypeSpecValue{Jackery HomePower 2000 Plus} \\
   \hline
   \HBTypeSpecLabel{Model No.} & \HBTypeSpecValue{JHP-2000A} \\
   \hline
   \HBTypeSpecLabel{Capacity} & \HBTypeSpecValue{2048 Wh (40 Ah/51.2V DC)} \\
   \hline
   \HBTypeSpecLabel{Cell Chemistry} & \HBTypeSpecValue{LiFePO₄} \\
   \hline
   \HBTypeSpecLabel{Weight} & \HBTypeSpecValue{About 41.45 lbs/18.8 kg} \\
   \hline
   \HBTypeSpecLabel{Dimensions} & \HBTypeSpecValue{14.4 × 10.0 × 10.7 in / 36.6 × 25.5 × 27.2 cm} \\
   \hline
   \HBTypeSpecLabel{Cycle Life} & \HBTypeSpecValue{6000 cycles to 70\%+ capacity} \\
   \end{tabularx}
   \end{spectable}

.. raw:: latex

   \specsectiontitle{INPUT PORTS}
   \begin{spectable}
   \noindent\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}m{\csname HBcomp_spec_table_left_ratio\endcsname\linewidth}|>{\raggedright\arraybackslash}X@{}}
   \HBTypeSpecLabel{1 × AC Input} & \HBTypeSpecValue{Charge Mode: 100V-120V\textasciitilde{}60Hz, 15A Max \newline Bypass Mode\HBSpecMarkerOne{}: 100V-120V\textasciitilde{}60Hz, 12A Max} \\
   \hline
   \HBTypeSpecLabel{2 × DC8020 Ports} & \HBTypeSpecValue{11V-16V⎓8A Max, Double to 8A Max \newline 16V-60V⎓12A Max, Double to 21A / 800W Max} \\
   \hline
   \HBTypeSpecLabel{1 × DC Expansion Port} & \HBTypeSpecValue{36.8 V-57.6 V⎓75 A max.} \\
   \end{tabularx}
   \end{spectable}

.. raw:: latex

   \specsectiontitle{OUTPUT PORTS}
   \begin{spectable}
   \noindent\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}m{\csname HBcomp_spec_table_left_ratio\endcsname\linewidth}|>{\raggedright\arraybackslash}X@{}}
   \HBTypeSpecLabel{4 × AC} & \HBTypeSpecValue{120V\textasciitilde{} 60Hz, 20A Max, 2400W Rated per port, 2400W in Total, 4800W Surge Peak} \\
   \hline
   \HBTypeSpecLabel{AC Output in Bypass Mode\HBSpecMarkerOne{}} & \HBTypeSpecValue{100V-120V\textasciitilde{}60Hz, 12A Max} \\
   \hline
   \HBTypeSpecLabel{1 × USB-A} & \HBTypeSpecValue{18W Max, 5-6V⎓3A, 6-9V⎓2A, 9-12V⎓1.5A} \\
   \hline
   \HBTypeSpecLabel{1 × DC 12 V Port} & \HBTypeSpecValue{12 V⎓10 A max.} \\
   \hline
   \HBTypeSpecLabel{1 × DC Expansion Port} & \HBTypeSpecValue{36.8 V-57.6 V⎓55 A max.} \\
   \end{tabularx}
   \end{spectable}

.. raw:: latex

   \specsectiontitle{ENVIRONMENTAL OPERATING TEMPERATURE}
   \begin{spectable}
   \noindent\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}m{\csname HBcomp_spec_table_left_ratio\endcsname\linewidth}|>{\raggedright\arraybackslash}X@{}}
   \HBTypeSpecLabel{Charging Temperature} & \HBTypeSpecValue{14°F to 113°F / -10°C to 45°C} \\
   \hline
   \HBTypeSpecLabel{Discharging Temperature} & \HBTypeSpecValue{14°F to 113°F / -10°C to 45°C} \\
   \end{tabularx}
   \end{spectable}



.. raw:: latex

   \vspace*{\csname HBcomp_spec_notes_before\endcsname}
   {\noindent
   \HBTypeSpecNote{※ USB Type-C® and USB-C® are registered trademarks of USB Implementers Forum.}\par
   }



.. raw:: latex

   \vspace*{\csname HBcomp_spec_footnotes_before\endcsname}
   {\noindent
   \HBTypeSpecNote{\HBSpecMarkerOne{} The product can charge the battery from the AC wall outlet while delivering power through the AC output ports.}\par
   }




.. only:: html

   .. raw:: html

      <h1 class="hb-h1-pill">SPECIFICATIONS</h1>

   .. raw:: html
   
      <h2 class="hb-spec-section"><span class="hb-spec-bullet" aria-hidden="true">&#9679;</span><span class="hb-spec-section-text">GENERAL INFO</span></h2>
      <table class="hb-spec-table">
        <tbody>
          <tr>
            <th scope="row" class="hb-spec-label">Product Name</th>
            <td class="hb-spec-value">Jackery HomePower 2000 Plus</td>
          </tr>
          <tr>
            <th scope="row" class="hb-spec-label">Model No.</th>
            <td class="hb-spec-value">JHP-2000A</td>
          </tr>
          <tr>
            <th scope="row" class="hb-spec-label">Capacity</th>
            <td class="hb-spec-value">2048 Wh (40 Ah/51.2V DC)</td>
          </tr>
          <tr>
            <th scope="row" class="hb-spec-label">Cell Chemistry</th>
            <td class="hb-spec-value">LiFePO₄</td>
          </tr>
          <tr>
            <th scope="row" class="hb-spec-label">Weight</th>
            <td class="hb-spec-value">About 41.45 lbs/18.8 kg</td>
          </tr>
          <tr>
            <th scope="row" class="hb-spec-label">Dimensions</th>
            <td class="hb-spec-value">14.4 × 10.0 × 10.7 in / 36.6 × 25.5 × 27.2 cm</td>
          </tr>
          <tr>
            <th scope="row" class="hb-spec-label">Cycle Life</th>
            <td class="hb-spec-value">6000 cycles to 70%+ capacity</td>
          </tr>
        </tbody>
      </table>
   
   .. raw:: html
   
      <h2 class="hb-spec-section"><span class="hb-spec-bullet" aria-hidden="true">&#9679;</span><span class="hb-spec-section-text">INPUT PORTS</span></h2>
      <table class="hb-spec-table">
        <tbody>
          <tr>
            <th scope="row" class="hb-spec-label">1 × AC Input</th>
            <td class="hb-spec-value">Charge Mode: 100V-120V~60Hz, 15A Max<br/>Bypass Mode①: 100V-120V~60Hz, 12A Max</td>
          </tr>
          <tr>
            <th scope="row" class="hb-spec-label">2 × DC8020 Ports</th>
            <td class="hb-spec-value">11V-16V⎓8A Max, Double to 8A Max<br/>16V-60V⎓12A Max, Double to 21A / 800W Max</td>
          </tr>
          <tr>
            <th scope="row" class="hb-spec-label">1 × DC Expansion Port</th>
            <td class="hb-spec-value">36.8 V-57.6 V⎓75 A max.</td>
          </tr>
        </tbody>
      </table>
   
   .. raw:: html
   
      <h2 class="hb-spec-section"><span class="hb-spec-bullet" aria-hidden="true">&#9679;</span><span class="hb-spec-section-text">OUTPUT PORTS</span></h2>
      <table class="hb-spec-table">
        <tbody>
          <tr>
            <th scope="row" class="hb-spec-label">4 × AC</th>
            <td class="hb-spec-value">120V~ 60Hz, 20A Max, 2400W Rated per port, 2400W in Total, 4800W Surge Peak</td>
          </tr>
          <tr>
            <th scope="row" class="hb-spec-label">AC Output in Bypass Mode①</th>
            <td class="hb-spec-value">100V-120V~60Hz, 12A Max</td>
          </tr>
          <tr>
            <th scope="row" class="hb-spec-label">1 × USB-A</th>
            <td class="hb-spec-value">18W Max, 5-6V⎓3A, 6-9V⎓2A, 9-12V⎓1.5A</td>
          </tr>
          <tr>
            <th scope="row" class="hb-spec-label">1 × DC 12 V Port</th>
            <td class="hb-spec-value">12 V⎓10 A max.</td>
          </tr>
          <tr>
            <th scope="row" class="hb-spec-label">1 × DC Expansion Port</th>
            <td class="hb-spec-value">36.8 V-57.6 V⎓55 A max.</td>
          </tr>
        </tbody>
      </table>
   
   .. raw:: html
   
      <h2 class="hb-spec-section"><span class="hb-spec-bullet" aria-hidden="true">&#9679;</span><span class="hb-spec-section-text">ENVIRONMENTAL OPERATING TEMPERATURE</span></h2>
      <table class="hb-spec-table">
        <tbody>
          <tr>
            <th scope="row" class="hb-spec-label">Charging Temperature</th>
            <td class="hb-spec-value">14°F to 113°F / -10°C to 45°C</td>
          </tr>
          <tr>
            <th scope="row" class="hb-spec-label">Discharging Temperature</th>
            <td class="hb-spec-value">14°F to 113°F / -10°C to 45°C</td>
          </tr>
        </tbody>
      </table>

   .. raw:: html
   
      <p class="hb-spec-note" data-spec-trailer-kind="note">※ USB Type-C® and USB-C® are registered trademarks of USB Implementers Forum.</p>

   .. raw:: html
   
      <p class="hb-spec-footnote" data-spec-trailer-kind="footnote">① The product can charge the battery from the AC wall outlet while delivering power through the AC output ports.</p>
