...
| UPSとして使用している際はパススルー機能により充電と出力が同時に行われている状態のため、ポータブル電源の最大出力は15Aに制限され、停電時には定格出力に戻ります。複数の機器をUPSモードで同時に接続する場合は、合計負荷電力がバイパス時の最大出力（15A）を超えないようにしてください。超えると、機器が過負荷保護により停止するおそれがあります。
