...
무정전 전원 공급 장치 (UPS)
===================================
| AC 충전 케이블로 제품을 벽면 콘센트에 연결한 다음, AC 전원 버튼을(를) 눌러 가전제품에 동시에 전원을 공급하십시오.
...
| 그리드 전력이 갑자기 중단되는 경우, Jackery Explorer 1000 Plus 리튬이차전지시스템은(는) 10 ms 이내에 저장된 전원으로 자동 전환되어 가전제품의 작동을 유지합니다.
| UPS 모드에서는 정전 발생 전 장치의 최대 출력이 8.18 A에 도달합니다. 바이패스 모드에서는 동시 충전/방전이 가능하므로,
